...
`Smart Energy Chain` is compatible with `Ethereum`'s ecosystem，support all `Ethereum`'s `RPC` API and DK。
...
async function getChainId() {
    const web3 = new Web3('https://testnet-rpc.secexplorer.io/')
...
async function transfer(fromAccount, to, value){
    const web3 = new Web3('https://rpc.secexplorer.io/')
    let chainId = await web3.eth.getChainId()
